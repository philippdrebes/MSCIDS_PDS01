...
class Student:
    ...
    def verify_registration_status(self):
        ...
        # print("status == 'registered': ", (status == 'registered'))
        self.status_verified = (status == "registered")        # you set (on runtime!)  a new instance variable!!!
        # print("self.status_verified: ", self.status_verified)

    def get_guardian_name(self ):
        return self.guardian

    def set_guardian_name(self, guardian_name):
        self.guardian = guardian_name                          # you set (on runtime!)  a new instance variable!!!

    def get_status(self):
        # get the registration status from a database (siumlation!!!)
        status = self.query_database(self.first_name, self.last_name)
        # print("status in 'get_status': ", status)
        return status
